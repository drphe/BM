...
function isTradingDay() { // kiểm tra xem có phải ngày giao dịch  không, ngày lễ thì trả về false;
    const holidayDates_solar = [
        "01-01", // Tết Dương lịch
        "30-04", // Ngày Giải phóng miền Nam
        "01-05", // Ngày Quốc tế Lao động
        "02-09", // Ngày Quốc khánh
    ];
    const holidayDates_lunar = [
        "01-01", // Tết Nguyên đán (âm lịch) 5 ngày
        "02-01", // Tết Nguyên đán (âm lịch)
        "03-01", // Tết Nguyên đán (âm lịch)
        "29-12", // Tết Nguyên đán (âm lịch)
        "30-12", // Tết Nguyên đán (âm lịch)
    ...
    const formattedDate_solar = `${dd.toString().padStart(2, "0")}-${mm.toString().padStart(2, "0")}`;
    const formattedDate_lunar = `${ld.day.toString().padStart(2, "0")}-${ld.month.toString().padStart(2, "0")}`;
    if (holidayDates_lunar.includes(formattedDate_lunar)) {
        return false;
    }
    if (holidayDates_solar.includes(formattedDate_solar)) {
        return false;
    }
    return day >= 1 && day <= 5;
}