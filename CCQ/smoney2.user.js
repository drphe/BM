...
(function() {
    ...
    async function fetchData(code, originalDiv) {
        ...
        try {
            ...
            for (let i = 1; i < temp.length; i++) {
                arrayData.push({ date: temp[i].startDate, price: temp[i].start, change: "▲" + temp[i - 1].recover.toFixed(2) + "%" });
                arrayData.push({ date: temp[i].bottomDate, price: temp[i].bottom, change: "▼" + temp[i].drawdown.toFixed(2) + "%" });
            }
    ...
    function renderChart(data) {
        ...
        if (chartDom) {
            ...
            const option = {
                ...
                xAxis: { type: 'category', show: true, data: categories },
                yAxis: { 
                    type: 'value', 
                    min: 'dataMin', 
                    max: 'dataMax',
                    axisLabel: { color: 'grey', formatter: value => Math.floor(value) }
                },
                tooltip: {
                    trigger: 'axis',
                    confine: true,
                    axisPointer: { type: 'cross', lineStyle: { color: 'grey', type: 'dashed' } },
                ...
                series: [
                    {
                        ...
                        data: trendlineData,
                        symbol: 'none',
                        lineStyle: { type: 'dashed', color: 'grey', width: 1, opacity: 0.6 },
                        silent: true
                    },
                    {
                        name: 'Giá',
    ...
    function findRecoveries(data) {
        ...
        let peak = data[0].value, tpeak = data[0].time;
        let trough = data[0].value, ttrough = data[0].time;
        let inDecline = false;

        for (let i = 1; i < data.length; i++) {
            if (!inDecline) {
                if (data[i].value < peak) { inDecline = true; trough = data[i].value; ttrough = data[i].time; }
                else { peak = data[i].value; tpeak = data[i].time; }
            } else {
                if (trough > data[i].value) { trough = data[i].value; ttrough = data[i].time; }
                else if (data[i].value > peak) { peak = data[i].value; tpeak = data[i].time; inDecline = false; }
                else {
                    if ((data[i].value > ma10[i] && trough < ma10[i]) || (data.length - i < 6)) {
                        results.push({
                            drawdown: (1 - trough / peak) * 100,
                            start: peak, startDate: formatTimestamp(tpeak),
                            bottom: trough, bottomDate: formatTimestamp(ttrough),
                            recover: 0,
                        });
                        inDecline = false; peak = data[i].value; tpeak = data[i].time;
                    }
                }
            }
        }
    ...
    function calculateConfidenceInterval(data) {
        if (!data.length) return { n: 0, mean: 0, lowerBound: 0, upperBound: 0 };
        const n = data.length;
        const mean = data.reduce((a, b) => a + b, 0) / n;
        const variance = data.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / (n - 1);
        const margin = 1.96 * (Math.sqrt(variance) / Math.sqrt(n));
        return { n, mean, lowerBound: mean - margin, upperBound: mean + margin };
    }
    ...
    function renderResults(results) {
        ...
        return `<div style="height:200px; overflow:auto;"><table border="1" style="width:100%; font-size:12px;">${rows}</table></div>`;
    }
