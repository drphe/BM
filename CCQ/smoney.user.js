...
// @version      1.2.7.3
// @description  Tính toán biến động NAV dự kiến dựa trên danh mục cổ phiếu của quỹ, so sánh quỹ cổ phiếu
// @author       Drphe
...
(function() {
    ...
    style.textContent = `#stock-ticker .symbol-stats .price {min-width: 120px;}`;
    document.head.appendChild(style);
    if (window.location.pathname.startsWith("/quy-dau-tu")) {
        let latestDateNav;
        let ckht = '';
        ...
        function getStockHistory(ticker) {
            return fetch(`https://api.simplize.vn/api/historical/prices/ohlcv?ticker=${ticker}&size=5&interval=1d&type=stock`).then(res => res.json()).then(result => {
        ...
        setTimeout(function() {
            ...
            if (target && symbol) {
                ...
                buttons.innerHTML = `<button class="" data-size="1N">1Y</button><button class="" data-size="3N">3Y</button><button class="" data-size="5N">5Y</button><button class="active" data-size="">All</button>`;
    ...
    // === Chức năng biểu đồ so sánh quỹ
    if (window.location.pathname.startsWith("/so-sanh-quy-dau-tu")) {
            ...
            if (typeof echarts !== 'undefined') {
                ...
                const option = {
                    ...
                    xAxis: {
                        ...
                        data: sortedDates,
                        axisLabel: {
                            formatter: (val) => val.split("-").slice(1).reverse().join("/")
                        }
                    },
                    yAxis: {
                        type: "value",
